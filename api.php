...
class epointsms{
    ...
    public function apiSettings(){
        $result = select_query("mod_epointsms_settings", "*","");
    ...
    function getReport($uid){
        $result = mysql_query("SELECT group_id FROM mod_epointsms_messages WHERE uid = '$uid' LIMIT 1");
    ...
    function checkLists($lists = null){
        ...
        foreach($lists as $lists){
            $sql = "SELECT `id` FROM `mod_epointsms_templates` WHERE `name` = '".$lists['function']."' AND `type` = '".$lists['type']."' LIMIT 1";
            ...
            if($num_rows == 0){
                if($lists['type']){
                    ...
                    insert_query("mod_epointsms_templates", $values);
                    $i++;
                }
            }else{
                $values = array(
                ...
                update_query("mod_epointsms_templates", $values, "name = '" . $lists['name']."'");
            }
        }
        return $i;
    }
	...
	function getUnverifiedOTP( $clientId ) {
		...
		$result = select_query( "mod_epointsms_otp", '', array( "relid" => $clientId , "type" => 'client' , 'status' => 1 ) );
	...
	function getOtp( $otpId ) {
		...
		$result = select_query( "mod_epointsms_otp", '', array( "id" => $otpId ) );
		return mysql_fetch_array( $result , MYSQL_ASSOC );		
	}
	
	function getClientOTP( $clientId ) {
		$result = select_query( "mod_epointsms_otp", '', array( "relid" => $clientId , "type" => 'client' ) );
		while($data = mysql_fetch_array( $result , MYSQL_ASSOC)) {
			$return_data[] = $data;
		}
