...
function epointsms_config() {
    $configarray = [
            "name" => "ePoint SMS",
            "description" => "ePoint SMS- WHMCS SMS Addon. To Create Account  <a href=\"https://epoint.com.bd\" target='_blank'>Epoint SMS Bangladesh</a>",
            "version" => "1.1.0",
            "author" => "<img src='https://bulk-sms.epoint.com.bd/public/admin/dist/img/logo.png' width='80'>",
            "language" => "english",
        ];
    return $configarray;
}

function epointsms_activate() {

    $query = "CREATE TABLE IF NOT EXISTS `mod_epointsms_messages` (`id` int(11) NOT NULL AUTO_INCREMENT,`group_id` varchar(40) NOT NULL,`to` varchar(15) DEFAULT NULL,`text` text,`uid` varchar(50) DEFAULT NULL,`status` varchar(10) DEFAULT NULL,`errors` text,`logs` text,`user` int(11) DEFAULT NULL,`datetime` datetime NOT NULL,PRIMARY KEY (`id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;";
	mysql_query($query);

    $query = "CREATE TABLE IF NOT EXISTS `mod_epointsms_settings` (`id` int(11) NOT NULL AUTO_INCREMENT,`api_key` varchar(100) CHARACTER SET utf8 NOT NULL,`api_token` varchar(500) CHARACTER SET utf8 NOT NULL,`sender_id` varchar(500) CHARACTER SET utf8 NULL,`wantsmsfield` int(11) DEFAULT NULL,`gsmnumberfield` int(11) DEFAULT NULL,`dateformat` varchar(12) CHARACTER SET utf8 DEFAULT NULL,`version` varchar(6) CHARACTER SET utf8 DEFAULT NULL,PRIMARY KEY (`id`)) ENGINE=MyISAM  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;";
	mysql_query($query);

    $query = "INSERT INTO `mod_epointsms_settings` (`api_key`, `api_token`,`sender_id`, `wantsmsfield`, `gsmnumberfield`,`dateformat`, `version`) VALUES ('none', 'none','DEMO_SMS', 0, 0,'%d.%m.%y','1.0.0');";
	mysql_query($query);

    $query = "CREATE TABLE IF NOT EXISTS `mod_epointsms_templates` (`id` int(11) NOT NULL AUTO_INCREMENT,`name` varchar(50) CHARACTER SET utf8 NOT NULL,`type` enum('client','admin') CHARACTER SET utf8 NOT NULL,`admingsm` varchar(255) CHARACTER SET utf8 NOT NULL,`template` varchar(240) CHARACTER SET utf8 NOT NULL,`variables` varchar(500) CHARACTER SET utf8 NOT NULL,`active` tinyint(1) NOT NULL,`extra` varchar(3) CHARACTER SET utf8 NOT NULL,`description` text CHARACTER SET utf8,PRIMARY KEY (`id`)) ENGINE=MyISAM  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;";
	mysql_query($query);

    $query = "CREATE TABLE IF NOT EXISTS `mod_epointsms_otp` (`id` int(11) NOT NULL AUTO_INCREMENT,`otp` varchar(50) CHARACTER SET utf8 NOT NULL,`type` enum('client','admin') CHARACTER SET utf8 DEFAULT 'client',`relid` int(10) DEFAULT 0,`request` varchar(50) CHARACTER SET utf8 NOT NULL,`text` text,`status` tinyint(1) DEFAULT 0, `datetime` datetime NOT NULL, `phonenumber` text, PRIMARY KEY (`id`)) ENGINE=MyISAM  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;";
	mysql_query($query);
    
    $query="
    INSERT INTO `mod_epointsms_templates` (`id`, `name`, `type`, `admingsm`, `template`, `variables`, `active`, `extra`, `description`) VALUES
    ...
    require_once("lib/epointsms.class.php");
    $api = new epointsms();
    $api->checkLists();

    return array('status'=>'success','description'=>'epointsms Addon successfully activated');
}

function epointsms_deactivate() {

    $query = "DROP TABLE `mod_epointsms_templates`";
	mysql_query($query);
    $query = "DROP TABLE `mod_epointsms_settings`";
    mysql_query($query);
    $query = "DROP TABLE `mod_epointsms_messages`";
    mysql_query($query);
	//DROP Table for OTP
    $query = "DROP TABLE `mod_epointsms_otp`";
    mysql_query($query);

    return array('status'=>'success','description'=>'epointsms Addon successfully deactivated');
}
...
function epointsms_output($vars){
    ...
    <div id="clienttabs">
        <ul>
            <li class="' . (($tab == "settings" || (@$_GET['type'] == "" && $tab == ""))?"tabselected":"tab") . '"><a href="addonmodules.php?module=epointsms&tab=settings">'.$LANG['settings'].'</a></li>
            <li class="' . ((@$_GET['type'] == "client")?"tabselected":"tab") . '"><a href="addonmodules.php?module=epointsms&tab=templates&type=client">'.$LANG['clientsmstemplates'].'</a></li>
            <li class="' . ((@$_GET['type'] == "admin")?"tabselected":"tab") . '"><a href="addonmodules.php?module=epointsms&tab=templates&type=admin">'.$LANG['adminsmstemplates'].'</a></li>
            <li class="' . (($tab == "sendbulk")?"tabselected":"tab") . '"><a href="addonmodules.php?module=epointsms&tab=sendbulk">'.$LANG['sendsms'].'</a></li>
            <li class="' . (($tab == "messages")?"tabselected":"tab") . '"><a href="addonmodules.php?module=epointsms&amp;tab=messages">'.$LANG['messages'].'</a></li>
            <li class="' . (($tab == "c")?"tabselected":"tab") . '"><a href="addonmodules.php?module=epointsms&amp;tab=support">'.$LANG['support'].'</a></li>
        </ul>
    </div>
    ...
    if (!isset($tab) || $tab == "settings")
    {
        ...
        if (isset($_POST['params'])) {
            ...
            update_query("mod_epointsms_settings", $update, "");
        }
        /* UPDATE SETTINGS */
...
<br><br>
        ...
        <input type="hidden" name="action" value="save" />
			...
			<input type="hidden" name="params" value="0"/>
                            ...
                            <td class="fieldlabel" width="30%">'.$LANG['clientid'].'</td>
                            <div class="input-group">
...
<br><br>
    ...
    elseif ($tab == "templates")
    {
        if (isset($_POST['params'])) { 
            ...
            $result = select_query("mod_epointsms_templates", "*", $where);
            while ($data = mysql_fetch_array($result)) {
                ...
                update_query("mod_epointsms_templates", $update, "id = " . $data['id']);
            }
        }
    ...
    elseif ($tab == "messages")
    {
        ...
        FROM `mod_epointsms_messages` as `m`
        ...
        $total = "SELECT count(id) as toplam FROM `mod_epointsms_messages`";
        ...
        while ($data = mysql_fetch_array($result)) {
            if($data['group_id'] && $data['status'] == ""){
                ...
                mysql_query("UPDATE mod_epointsms_messages SET status = '$status' WHERE id = ".$data['id']);
            }else{
                $status = $data['status'];
            }
            ...
            <td><center><a href="addonmodules.php?module=epointsms&tab=messages&deletesms='.$data['id'].'" title="'.$LANG['delete'].'"><img src="images/delete.gif" width="16" height="16" border="0" alt="Delete"></a></center></td></tr>';
        }
        /* Getting messages order by date desc */
        ...
        for($a=1;$a<=$sayfa;$a++)
        {
            ...
            $list.="<option value='addonmodules.php?module=epointsms&tab=messages&page={$a}&limit={$limit}&order={$order}' {$selected}>{$a}</option>";
        }
